...
public actor CloudOpenAIBackend: LLMBackend {
    public nonisolated let provider: LLMProvider = .openAI
    ...
    private struct OpenAIRequest: Encodable {
            ...
            func encode(to encoder: Encoder) throws {
                ...
                case .parts(let parts):
                    ...
                    for part in parts {
                        switch part {
                        case .text(let text):
                            try arr.encode(ContentPartJSON(type: "text", text: text))
                        case .imageURL(let url):
                            try arr.encode(ContentPartJSON(type: "image_url", imageURL: .init(url: url.absoluteString)))
                        }
                    }
                }
            }
        ...
        struct ContentPartJSON: Encodable {
            let type: String
            var text: String?
            var imageURL: ImageURLJSON?
